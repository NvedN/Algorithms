...
public class finalTaskA {
   ...
   * -- ПРИНЦИП РАБОТЫ --
   * <p>
   * В данной реализации используется кольцевой буфер, который представляет собой примитивный
   ...
   * <p>
   * Для доказательства корректности работы данной реализации необходимо доказать, что все операции
   * push и pop выполняются корректно и не нарушают инварианты кольцевого буфера и свойства дека.
   * Операции push и pop выполняются корректно и не нарушают инварианты кольцевого буфера и свойства
   * дека, поскольку: Операции pushFront и pushBack добавляют элементы в дек, сдвигая указатели
   * front и back соответственно. Если дек заполнен, будет выброшено исключение. Операции popFront и
   * popBack удаляют элементы из дека, сдвигая указатели front и back соответственно. Если дек пуст,
   * будет выброшено исключение. Инварианты кольцевого буфера сохраняются после каждой операции push
   * и pop, так как указатели front и back сдвигаются по кольцевому буферу вправо и влево, а
   * значения переменных front и back всегда остаются в пределах длины буфера. Свойства дека
   * сохраняются после каждой операции push и pop, так как элементы дека упорядочены и добавляются в
   * конец или начало дека в зависимости от выбранной операции. Также дек может содержать
   * произвольное количество элементов в пределах максимального размера, заданного при
   * инициализации.
   * <p>
   * -- ВРЕМЕННАЯ СЛОЖНОСТЬ --
   * <p>
   * Временная сложность алгоритма для всех операций push и pop в данной реализации равна O(1), так
   * как все операции выполняются за постоянное время, независимо от количества элементов в деке.
   * Операции push и pop выполняются за константное время, поскольку индексы front и back
   * обновляются по кольцевому буферу, и не требуют перемещения элементов в массиве.
   * <p>
   * -- ПРОСТРАНСТВЕННАЯ СЛОЖНОСТЬ --
   * <p>
   * Пространственная сложность алгоритма реализации дека с использованием кольцевого буфера и

...
public class FinalB {
   ...
   *
   * <p>-- ВРЕМЕННАЯ СЛОЖНОСТЬ -- Временная сложность алгоритма также зависит от высоты дерева. В худшем случае, когда дерево
   * является несбалансированным, время выполнения будет O(n), где n - количество узлов в дереве. В
   * сбалансированном дереве время выполнения будет O(log n).
   *
   * <p>-- ПРОСТРАНСТВЕННАЯ СЛОЖНОСТЬ -- Пространственная сложность алгоритма равна O(h), где h -
   * высота дерева. Это связано с тем, что в худшем случае в стеке вызовов метода remove() будет
   * находиться количество элементов, равное высоте дерева.
   */
